...
class SendMessageTool(Tool):
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        if team is None:
            return ToolResult(
                output=f"Team '{self._team_name}' not found", is_error=True
            )
        ...
        if p.to == "*":
            member_ids = [
                m.agent_id for m in team.members if m.agent_id != self._from_agent_id
            ]
            ...
            return ToolResult(
                output=f"Message broadcast to {len(member_ids)} teammates."
            )
    ...
    def _wake_pane(self, agent_id: str) -> None:
        ...
        if pane_id is None:
            return
        try:
            from kova.teams.spawn_tmux import send_keys_to_pane
