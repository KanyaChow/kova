...
class AgentTool(Tool):
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        from kova.agents.fork import ForkError, build_forked_messages
        from kova.agents.parser import AgentDef
        from kova.agents.tool_filter import (
            clone_registry_for_fork,
            resolve_agent_tools,
        )
        from kova.agent import Agent as AgentClass
        ...
        if p.subagent_type is None:
            from kova.context import clone_replacement_state
    ...
    async def _execute_as_teammate(self, p: AgentToolParams) -> ToolResult:
        ...
        from kova.agents.fork import ForkError, build_forked_messages
        from kova.agents.parser import AgentDef
        from kova.agents.tool_filter import build_teammate_tools
        from kova.agent import Agent as AgentClass
        from kova.conversation import ConversationManager
        from kova.permissions import (
            DangerousCommandDetector,
        ...
        from kova.teams.models import BackendType, TeammateInfo
    ...
    def _spawn_pane_teammate(
        ...
        from kova.teams.spawn import build_teammate_cli

        # 外部进程通过邮箱领取初始任务：spawn 前先把任务投进队友邮箱（按队友名字为键），
        # 新进程启动后第一次空闲轮询就能看到工作。对齐 Go 的 SpawnTeammate。
        mailbox = self._team_manager.get_mailbox(p.team_name)
        if mailbox is not None and p.prompt:
            from kova.teams.mailbox import create_message
            from kova.teams.spawn_inprocess import LEAD_NAME
        ...
        # 构造把本 kova 拉起为队友 worker 模式的命令，cd 到该队友的 worktree
        cli_command = build_teammate_cli(p.team_name, teammate_name, wt.path)

        try:
            if backend == BackendType.TMUX:
                from kova.teams.spawn_tmux import spawn_tmux_teammate
                ...
                from kova.teams.spawn_iterm2 import spawn_iterm2_teammate
    ...
    async def _execute_with_worktree(self, p: AgentToolParams) -> ToolResult:
        ...
        from kova.worktree.integration import (
    ...
    def _create_client_for_model(self, model_alias: str) -> LLMClient | None:
        ...
        from kova.client import create_client
        from kova.config import ProviderConfig
