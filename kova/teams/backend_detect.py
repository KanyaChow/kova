...
def detect_backend_from_env() -> BackendType:
    ...
    对齐 Claude Code：tmux 和 iTerm2 会自动给会话内的进程设上 TMUX /
    ITERM_SESSION_ID 环境变量，用户无需手动配置。只有已经身处这类会话里，
    才把队友放进独立窗格；否则一律进程内运行。
    """
...
def detect_pane_backend(
    ...
    对齐 Claude Code：不做“检测到系统装了 tmux 就用窗格”的探测，
