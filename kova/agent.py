...
from kova.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from kova.conversation import ThinkingBlock as ConvThinkingBlock
...
from kova.hooks import HookContext, HookEngine, ToolRejectedError
from kova.hooks.engine import HookNotification
...
class Agent:
    def __init__(
        ...
        if memory_manager is not None:
            from kova.memory.consolidation import MemoryConsolidator
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from kova.permissions.rules import Rule, extract_content
    ...
    async def run_to_completion(
        ...
        if conversation is None:
            ...
            if self.instructions_content:
                memory_content = (
                    self.memory_manager.load() if self.memory_manager else ""
                )
                conversation.inject_long_term_memory(
                    self.instructions_content, memory_content
                )
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from kova.context.manager import (
