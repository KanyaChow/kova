...
from kova.hooks import HookConfigError, HookEngine, load_hooks
from kova.permissions import PermissionMode


def main() -> None:
    # 队友 worker 模式：由 tmux/iTerm2 窗格用 `-m kova --teammate ...` 拉起。
    ...
    parser = argparse.ArgumentParser(
        prog="kova", description="Kova AI coding assistant"
    )
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from kova.remote import RemoteServer
    ...
    from kova.app import KovaApp
    from kova.driver import NoAltScreenDriver
...
async def _run_prompt(
    ...
    from kova.client import create_client, resolve_context_window
    from kova.conversation import ConversationManager
    from kova.memory.instructions import load_instructions
...
async def _run_teammate(team_name: str, agent_name: str) -> None:
    ...
    from kova.teams.spawn_inprocess import LEAD_NAME, spawn_inprocess_teammate
    from kova.tools import create_default_registry
    from kova.tools.send_message import SendMessageTool
